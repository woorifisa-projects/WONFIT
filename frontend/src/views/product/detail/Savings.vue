    ...
    <div>
      ...
      <div>
        <v-sheet elevation="4" max-width="900" rounded="lg" width="100%" class="pa-6 mt-10 mx-auto">
          ...
          <detail-text dataName="개요" :dataContent="savingsData.savingsInfo" />
          <detail-text
            dataName="예금자 보호"
...
초과하는 나머지 금액은 보호하지 않습니다.'
          />
          <detail-text dataName="가입 대상" :dataContent="savingsData.target" />
          <detail-text dataName="적립 금액" :dataContent="savingsData.maxDeposit + '원'" />
          <detail-text dataName="가입 기간" :dataContent="savingsData.period + '개월'" />
          <detail-text dataName="기본 금리" :dataContent="savingsData.interestRate + '%'" />
          <detail-text
            dataName="우대 금리"
            dataContent="
최대 연 0.2%p
