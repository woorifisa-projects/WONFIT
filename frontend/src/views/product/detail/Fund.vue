<template>
    ...
    <div>
      ...
      <div>
        <v-sheet elevation="4" max-width="900" rounded="lg" width="100%" class="pa-6 mt-10 mx-auto">
          ...
          <detail-text dataName="개요" :dataContent="fundData.fundInfo" />
          <detail-text
            dataName="예금자 보호"
  ...
  초과하는 나머지 금액은 보호하지 않습니다.'
          />
          <detail-text dataName="기준가" :dataContent="fundData.fundPrice + '원'" />
          <detail-text dataName="1개월 금리" :dataContent="fundData.returnRate1 + '%'" />
          <detail-text dataName="6개월 금리" :dataContent="fundData.returnRate2" />
          <detail-text
            dataName="우대 금리"
            dataContent="
  최대 연 0.2%p
