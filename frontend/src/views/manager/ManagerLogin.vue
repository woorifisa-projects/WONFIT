      ...
      <v-form @submit.prevent="login">
        ...
        <v-card-text class="text-center">
          <a class="text-blue text-decoration-none" href="#" rel="noopener noreferrer">
            Manager Signup <v-icon icon="mdi-chevron-right"></v-icon>
          </a>
        </v-card-text>
      </v-form>
    </v-card>
...
// TODO: managerSignup page 만들기
// const navigateToSignup = () => {
//   router.push({ name: "managerSignup" });
// };
...
const login = async () => {
  try {
    ...
    const response = await axios.post("https://back.wonfit.site/manager/login", requestBody, {
      // const response = await axios.post("http://localhost:8080/manager/login", requestBody, {
